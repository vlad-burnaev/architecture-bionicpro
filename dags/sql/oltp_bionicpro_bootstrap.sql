-- Выполняется через PostgresOperator (conn → БД sample).
-- Нужно для томов Postgres, созданных до появления db/02-sample-oltp.sql.
...
-- sub = id пользователей prothetic1 / prothetic2 в keycloak/realm-export.json
